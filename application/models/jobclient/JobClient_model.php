 ...
 */
class JobClient_model extends CI_Model {
	...
	public function getAllJob($data)
	{
		...
		$this->db->select('concat(jc.first_name," ",jc.last_name) as contact_name,jc.mobile,(jc.id) as job_contact_id,j.job_name,j.status,(j.id) as job_id,(p.name) as patient_name,p.caller,p.created_date,(p.id) as patient_id');
