...
<th width="30%;text-align:left"><i class="fa fa-cogs text-success" aria-hidden="true"> </i>   Action </th>
...
<td><a href="<?php echo base_url();?>job_detail/<?php echo $row['id'];?>"><span class="tag bg-red"> <?php echo getJobID($row['id']);?>  </span> </a></td>
...
<td> <?php if($row['assign_to'] == "" or $row['assign_to'] == NULL) echo "Not Assigned"; else echo $row['assign_to'];?> </td>
   ...
   <!-- endbuild -->
    ...
    $('.datatable').DataTable();
      ...
      function update_dataTable(data,tableid) {
        ...
        var job_id = '<a href="<?php echo base_url();?>job_detail/'+data[index].id+'"><span class="tag bg-red"> '+data[index].job_id+'  </span> </a>';
    	  	var row = [];
    	    	row.push(map);
    	    	row.push(edit);
    	    	row.push(job_id);
    	...
    	function action_filter () {
    		$.post("<?php echo base_url();?>admin/filter_job",{"action": $('#action').val(),"branch": $('#branch').val(),"company_id":'<?php echo $global_company_id;?>'},function(data){
    			tableid="table_id1";
    			update_dataTable(data,tableid);
